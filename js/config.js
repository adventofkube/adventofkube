...
export const DAYS = [
  ...
  },
  {
    ...
    enabled: true,
    title: 'App of Apps',
    description: 'An ArgoCD App of Apps is broken. The parent Application points to the wrong path in the source repository, so the child Application and its workload are never deployed. Restore the GitOps chain.',
    flagHash: 'caef9c654e06a53987e52e1eba02a38ecb543cd10c88d27287460d6c470edd84',
    setup: [
      'Follow the <a href="https://argo-cd.readthedocs.io/en/stable/getting_started/" target="_blank" rel="noopener">ArgoCD docs</a> to set up ArgoCD on your cluster, forward the server to your localhost, and log in to the ArgoCD GUI using the password stored in the generated secret.',
      'Install the chart:',
      '<code>helm install day21 oci://ghcr.io/adventofkube/charts/day21 --version 0.1.0</code>',
      'Open the ArgoCD UI and inspect the <code>gitops-app</code> Application.',
      'Once the workload is running, get the flag:',
      '<code>kubectl port-forward svc/gitops-app -n day21 8080:80\ncurl localhost:8080</code>',
    ],
    hints: [
      'The parent Application shows a sync error in ArgoCD. What path is it trying to sync from?<code>kubectl describe application gitops-app -n argocd</code>',
      'The path <code>deploy/day21/apps</code> doesn\'t exist in the source repository. Browse the repository link in the sidebar to find where the app manifests actually live.',
      'This is a Helm-managed resource — <code>kubectl edit</code> won\'t stick. Fix it through Helm instead:<code>helm upgrade day21 oci://ghcr.io/adventofkube/charts/day21 --version 0.1.0 --set argocd.path=manifests/day21/apps</code>Once the parent syncs, watch for a child Application <code>day21-workload</code> to appear and deploy the workload.',
    ],
    docs: [
      { title: 'adventofkube — Source Repository', url: 'https://github.com/adventofkube/adventofkube' },
      { title: 'ArgoCD App of Apps', url: 'https://argo-cd.readthedocs.io/en/stable/operator-manual/cluster-bootstrapping/' },
